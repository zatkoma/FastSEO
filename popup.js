document.addEventListener('DOMContentLoaded', function () {
    ...
    function seznam_ready(){
        ...
        chrome.storage.sync.get(["seznam"], function(result) {
            if(result.seznam === true){
...
// <000> - Začátek funkcí
    ...
    function seznam_create(){
        ...
        chrome.contextMenus.create({
            title: "URL in performance report",
            id: "90",
            contexts:["all"],
            onclick: urlGSCURL
        });
    ...
    //chrome.extension.getBackgroundPage().console.log('Prošlo');
    //chrome.extension.getBackgroundPage().console.log(document.getElementById("chck_seznam").getAttribute("name"));
    //chrome.storage.sync.get(["seznam"], function(result) {chrome.extension.getBackgroundPage().console.log(result.seznam);});
    //chrome.storage.sync.clear();
    //chrome.storage.sync.get(null, function(items) {var allKeys = Object.keys(items);chrome.extension.getBackgroundPage().console.log(allKeys);});

    chrome.storage.sync.get(["seznam","marketingminer","collabim","collabim_account","ahrefs","majestic","utility"], function(result) {
        if(result.seznam === false){
            document.getElementById('chck_seznam').removeAttribute("checked");
        }
        else
        {
            document.getElementById('chck_seznam').setAttribute("checked", "checked");
        }

        if(result.marketingminer === false){
            document.getElementById('chck_marketingminer').removeAttribute("checked");
        }
        else
        {
            document.getElementById('chck_marketingminer').setAttribute("checked", "checked");
        }

        if(result.ahrefs === false){
            document.getElementById('chck_ahrefs').removeAttribute("checked");
        }
        else
        {
            document.getElementById('chck_ahrefs').setAttribute("checked", "checked");
        }

        if(result.utility === false){
            document.getElementById('chck_utility').removeAttribute("checked");
        }
        else
        {
            document.getElementById('chck_utility').setAttribute("checked", "checked");
        }


    });

    var chechkbox_miner = document.getElementById("chck_marketingminer");
    chechkbox_miner.addEventListener("change", function(){
        if(this.checked) {
            chrome.storage.sync.set({"marketingminer": true}, function() {});
    ...
    var chechkbox_ahrefs = document.getElementById("chck_ahrefs");
    chechkbox_ahrefs.addEventListener("change", function(){
        if(this.checked) {
            chrome.storage.sync.set({"ahrefs": true}, function() {});
    ...
    var chechkbox_seznam = document.getElementById("chck_seznam");
    chechkbox_seznam.addEventListener("change", function(){
        if(this.checked) {
            ...
            chrome.storage.sync.set({"seznam": false}, function() {});

            aktualizujmenu();
        }
    });

    var chechkbox_utility = document.getElementById("chck_utility");
    chechkbox_utility.addEventListener("change", function(){
        if(this.checked) {
            ...
            chrome.storage.sync.set({"utility": false}, function() {});
    ...
    feedback.addEventListener("click", function(){
        chrome.tabs.create({url: "https://forms.gle/zjJLZ8ziKaq3ygkm6"});
    });
